...
if ($('.slider-lots__body').length > 0) {
	$('.slider-lots__body').slick({
		...
		//appendDots:
		//appendArrows:$('.mainslider-arrows .container'),
		nextArrow: $('.control-slider-lots__arrow-next'),
		prevArrow: $('.control-slider-lots__arrow-prev'),
		responsive: [{
			breakpoint: 768,
			settings: {
				slidesToShow: 2
...
const iconMenu = document.querySelector(".icon-menu"),
	...
	menuLink = document.querySelectorAll('.goto'),
	menuLinks = document.querySelector('.menu__list'),
	scrollItems = document.querySelectorAll('.to__scroll'),
...
const userIcon = document.querySelector('.user-header__icon'),
	userMenu = document.querySelector('.user-header__menu');
...
// переделка img в background для большей гибкости:
function ibg() {
...
// прокрутка для меню любого разрешения:
function addClassActive(i = 0) {
	menuLink[i].classList.add('active');
	const targetBlockClass = menuLink[i].getAttribute('href').replace('#', ''),
		targetBlock = document.querySelector('.' + targetBlockClass),
		elementPosition = targetBlock.getBoundingClientRect().top,
		offsetPosition = elementPosition - header;
	window.scrollBy({
		top: offsetPosition,
		behavior: 'smooth'
	});
...
// function removeClassActive() {  - эта функция нужна если нет функции для слушателя скролла
...
function clickOnMenuLink(event) {
	const target = event.target;
	if (target && target.classList.contains('goto')) {
		menuLink.forEach((item, i) => {
			if (target == item) {
				// removeClassActive(); - эта функция нужна если нет функции для слушателя скролла
				addClassActive(i);
				removeClasses();
			}
		});
...
menuLinks.addEventListener('click', clickOnMenuLink);
...
window.addEventListener('scroll', () => {
	const oldCurrentLink = document.querySelectorAll('.goto.active');
	if (oldCurrentLink) {
		oldCurrentLink.forEach(item => {
			item.classList.remove('active');
		});
	}
	scrollItems.forEach((item, i) => {
		const top = item.offsetTop - header - 5,
			bottom = top + item.clientHeight,
			scrolling = window.scrollY;
		if (scrolling > top && scrolling < bottom) {
			menuLink[i].classList.add('active');
		}
		if (scrolling + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
			menuLink[scrollItems.length - 2].classList.remove('active');
			menuLink[scrollItems.length - 1].classList.add('active');
		}
	});
});
